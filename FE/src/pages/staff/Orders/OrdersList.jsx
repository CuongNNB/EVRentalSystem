  ...
  const handleAction = async (order, action) => {
    ...
    switch (action) {
      ...
      case "handover":
        ...
        break;
      case "invoice":
        // Chỉ cập nhật trạng thái, không chuyển trang
        if (updateSucceeded) {
          setConnectionState({
            status: "success",
            message: `Đã nhận xe cho đơn ${order.id}. Xe đã được kiểm tra.`,
          });
        } else {
          setConnectionState({
            status: "warning",
            message: `Đã nhận xe cho đơn ${order.id} (tạm thời do lỗi kết nối, vui lòng kiểm tra lại).`,
          });
        }
        break;
      case "revenue":
